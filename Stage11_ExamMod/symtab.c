gsymbol* GListInstall(astnode* list, typetable* type){
	...
	while(tmp!=NULL){
		if(type==TLookup(strdup("PAIR")))
			GInstall(tmp->name,type,2*tmp->value,tmp->ptr2);
		else
...
lsymbol* LListInstall(astnode *list, typetable* type){
	...
	while(tmp!=NULL){
		if(type==TLookup(strdup("PAIR")))
			LInstall(tmp->name,type,2*tmp->value);
		else
...
lsymbol* LInstallArg(astnode *list){
	...
	while(iter!=NULL){
		...
		tmp->binding=larg-1;
		larg-=1;
		if(iter->type==TLookup(strdup("PAIR")))
			larg-=1;
		tmp->next=lroot;
		lroot=tmp;
		iter=iter->ptr1;
...
void LargAssign(typetable* type){
	...
	while(tmp!=NULL){
		if(tmp->binding<0){
			if(tmp->type==TLookup(strdup("PAIR")))
				tmpsym--;
			tmp->binding=tmpsym--;
		}
		tmp=tmp->next;
